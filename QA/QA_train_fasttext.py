...
import time
from utils import normalize_qa, get_similarity
import transformers
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="HOw many relevant articles to keep",
    )
...
def main(args):
    ...
    def preprocess_function(examples):
        ...
        for id in similarity_dict:
            ...
            K = round(len(passagesForOne)*keep_ratio)
            for k in range(K):
                try:
                ...
                except: # if no enough K can be returned
                    break 
            # print(id, candidates)
            # print(to_remove)
    ...
    def preprocess_test_function(examples):
        ...
        tokenized_inputs = {k:[[tokenized_examples['A'][k][i],
                                tokenized_examples['B'][k][i],
                                tokenized_examples['C'][k][i]] for i in range(len(sample_mapping))] for k in keys if k != 'overflow_to_sample_mapping'}
        tokenized_inputs["labels"] = [] # 0 or 1 
        tokenized_inputs["example_id"] = []
        for sample_index in sample_mapping:
            # Grab the sequence corresponding to that example (to know what is the context and what is the question).
